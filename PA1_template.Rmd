...
1. Waking hours appear to be shortly after 5 am.
...
days.all <- max(activity.cleaned.ds$date) - min(activity.cleaned.ds$date) + 1
```


Let's compare the histogram of daily (total) steps taken across
the `r days.all` day (cleaned) dataset
...
Key compartive observations:

1. Waking hours appear to be shortly after 5 am for both weekdays and weekends,
